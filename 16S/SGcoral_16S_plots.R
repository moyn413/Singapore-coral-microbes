#----------------------------------------------------------------
# 16S phyloseq processing file and script to make plots with nifH data. Starting file is a phyloseq file made using the dada2 pipeline.
...
# Separate phyloseqs based on sample types
ps.coral.all.norm <-  ps.all.norm %>% subset_samples(Type != "Seawater")
ps.coral.dna.norm <-  ps.all.norm %>% subset_samples(Type != "Seawater") %>% subset_samples(NucleicType == "DNA")
ps.coral.rna.norm <-  ps.all.norm %>% subset_samples(Type != "Seawater") %>% subset_samples(NucleicType == "RNA")
ps.SW.norm <-  ps.all.norm %>% subset_samples(Type == "Seawater")


# Separate phyloseqs based on sample types
ps.coral.all.abund <-  ps.all.abund %>% subset_samples(Type != "Seawater")
ps.coral.dna.abund <-  ps.all.abund %>% subset_samples(Type != "Seawater") %>% subset_samples(NucleicType == "DNA")
ps.coral.rna.abund <-  ps.all.abund %>% subset_samples(Type != "Seawater") %>% subset_samples(NucleicType == "RNA")
ps.SW.abund <-  ps.all.abund %>% subset_samples(Type == "Seawater")
